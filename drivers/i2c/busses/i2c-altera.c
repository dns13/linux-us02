...
static inline u8 altr_i2c_8bit_addr_from_msg(const struct i2c_msg *msg)
{
 ...
 */
static inline void altr_i2c_reinit_completion(struct completion *x)
{
	x->done = 0;
}
...
static int altr_i2c_xfer_msg(struct altr_i2c_dev *idev, struct i2c_msg *msg)
{
	...
	u32 value;
	u8 addr = altr_i2c_8bit_addr_from_msg(msg);
	...
	idev->buf = msg->buf;
	idev->msg_err = 0;
	altr_i2c_reinit_completion(&idev->msg_complete);
	altr_i2c_core_enable(idev);
